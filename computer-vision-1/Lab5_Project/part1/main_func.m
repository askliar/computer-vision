function main_func(temp_dir, num_train_idx, num_neg_idx, cluster_idx, sift_idx, colorspace_idx, sift_step_idx, sift_size_idx)
    if ~exist(sprintf('%s/data', temp_dir), 'dir')
        mkdir(sprintf('%s/data', temp_dir))
    end 

    if ~exist(sprintf('%s/data/html', temp_dir), 'dir')
        mkdir(sprintf('%s/data/html', temp_dir))
    end 

    data_dir = sprintf('%s/Caltech4/ImageData', temp_dir);
    ...
    num_vocab = 250;
    num_train_idx = str2num(num_train_idx);
    num_neg_idx = str2num(num_neg_idx);
    cluster_idx = str2num(cluster_idx);
    sift_idx = str2num(sift_idx);
    colorspace_idx = str2num(colorspace_idx);
    ...
    save(sprintf('%s/data/%d_%d_%d_%s_%s_%d_%d_test_names.mat', '.', num_train, num_neg, num_clusters, sift_method, colorspace, sift_step, sift_size), 'test_names');
    
    disp('saved data');

%     data = load(sprintf('%s/data/%d_%d_%d_%s_%s_%d_%d_data.mat', '.', num_train, num_neg, num_clusters, sift_method, colorspace, sift_step, sift_size));
%     data = data.data;
%     
%     labels = load(sprintf('%s/data/%d_%d_%d_%s_%s_%d_%d_labels.mat', '.', num_train, num_clusters, sift_method, colorspace, sift_step, sift_size));
%     labels = labels.labels;
%     
%     test_data = load(sprintf('%s/data/%d_%d_%d_%s_%s_%d_%d_test_data.mat', '.', num_train, num_neg, num_clusters, sift_method, colorspace, sift_step, sift_size));
%     test_data = test_data.test_data;
%     
%     test_labels = load(sprintf('%s/data/%d_%d_%d_%s_%s_%d_%d_test_labels.mat', '.', num_train, num_neg, num_clusters, sift_method, colorspace, sift_step, sift_size));
